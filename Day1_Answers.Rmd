...
1. Visualizing, summarizing, and standarizing data before modeling
...
This is the "Medical Cost Personal Datasets" dataset and it was downloaded from [Kaggle](https://www.kaggle.com/mirichoi0218/insurance). The motivation is to predic the insurance costs based on individuals' demographics and health conditions. The file has the following columns:
...
Once the data have been inspected, cleaned, and standarized, we can start estimating models. The simplest models (but those with the most assumptions) are those for continuous and unbounded outcomes. We use a linear regression model estimated using Ordinary Least Lquares (OLS), which in R can be fit with the `lm()` (linear model) function.
