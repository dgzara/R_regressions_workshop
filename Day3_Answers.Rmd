...
Given $m$ variables in a dataset, we can *add* or *remove* a predictor in the linear regresison model. Therefore, we can build $2^m-1$ models excluding the null model (no predcitors). For example, we can build $2^{12}-1=4095$ models for the wine dataset. 
...
We load the Healthcare insurace dataset as `health.data`.
```{r}
...
Another way of presenting the same information for adjusted $R^2$. The model with variables (counting dummy variables seprately) at the highest y-position has the highest adjusted $R^2$.
...
```{r}
regsubsets.out <- regsubsets(quality ~ ., wine.data, method = "exhaustive", nbest=2)
```
...
Identify the model with the hightest adj $R^2$. You can use the function `which.max()` on `summary.out$adjr2`
...
```{r}
wine.best.model <- lm(quality ~ volatile_acidity + citric_acid + chlorides + free_sulfur_dioxide + total_sulfur_dioxide + pH + sulphates + alcohol, wine.data)
summary(wine.best.model)
```
...
```{r,results='hide'}
fitall.out <- fitall(y = wine.data$quality,
                     x = wine.data[,c("fixed_acidity", "volatile_acidity", "citric_acid", "residual_sugar",
...
```{r}
backward.selection <- step(lm(log(quality) ~ ., wine.data), direction="backward")
```

Now, create the forward stepwise selector. Remember to update the dataframe, the formula, and the dependent variable (`quality`).
```{r}
min.model <- lm(log(quality) ~ 1, wine.data)
forward.selection <- step(min.model, direction="forward", scope = ~ fixed_acidity + volatile_acidity + citric_acid + residual_sugar + chlorides + free_sulfur_dioxide + total_sulfur_dioxide + density + pH + sulphates + alcohol)
forward.selection
```
